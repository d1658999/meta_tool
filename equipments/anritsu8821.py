    ...
    def get_power_aclr_evm_lte(self):
        ...
        for mod in want_mods:
            ...
            if ext_pmt.fdd_tdd_cross_test == 1:
                if mod in ['TX_MAXPWR_Q_1', 'TX_MAXPWR_Q_P', 'TX_MAXPWR_Q_F']:
                    self.set_test_parameter(mod)
                    self.set_ulrmc_64QAM('ENABLED')
                    self.set_ulrmc_256QAM('ENABLED')
                    self.set_ulmcs(5)

                elif mod in ['TX_MAXPWR_16_P', 'TX_MAXPWR_16_F']:
                    self.set_test_parameter(mod)
                    self.set_ulrmc_64QAM('ENABLED')
                    self.set_ulrmc_256QAM('ENABLED')
                    self.set_ulmcs(13)

                elif mod in ['TX_MAXPWR_64_P', 'TX_MAXPWR_64_F']:
                    self.set_test_parameter(mod)
                    self.set_ulrmc_64QAM('ENABLED')
                    self.set_ulrmc_256QAM('ENABLED')
                    self.set_ulmcs(22)

                elif mod in ['TX_MAXPWR_256_F']:
                    self.set_test_parameter(mod)
                    self.set_ulrmc_64QAM('ENABLED')
                    self.set_ulrmc_256QAM('ENABLED')
                    self.set_ulmcs(28)
            ...
            if mod == 'TX_MAXPWR_Q_1':  # mod[10:] -> Q_1
                ...
                self.anritsu_query('*OPC?')
            else:  # mod[10:] -> Q_P, Q_F, 16_P, 16_F, 64_F, 256_F
                logger.info(mod)

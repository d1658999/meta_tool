...
import utils.external_paramters as ex_pmt
...
class AtCmd:
    ...
    def set_duty_cycle(self):
        logger.info(f'----------Set duty cycle: {ex_pmt.duty_cycle}----------')
        if self.band_fr1 in [34, 38, 39, 40, 41, 42, 48, 75, 76, 77, 78, 79]:
            self.uldl_period = self.duty_cycle_dict[ex_pmt.duty_cycle][0]
            self.dl_slot = self.duty_cycle_dict[ex_pmt.duty_cycle][1]
            self.dl_symbol = self.duty_cycle_dict[ex_pmt.duty_cycle][2]
            self.ul_slot = self.duty_cycle_dict[ex_pmt.duty_cycle][3]
            self.ul_symbol = self.duty_cycle_dict[ex_pmt.duty_cycle][4]
    ...
    def tx_set_no_sync_fr1(self):
        ...
        logger.info(
            f'TX_PATH: {self.tx_path}, BW: {self.bw_fr1}, TX_FREQ: {self.tx_freq_fr1}, RB_SIZE: {self.rb_size_fr1}, '
            f'RB_OFFSET: {self.rb_start_fr1}, MCS: {self.mcs_fr1}, TX_LEVEL: {self.tx_level}, '
            f'Duty cycle: {ex_pmt.duty_cycle} %')
